...
module.exports = {
    getAll: async function(req, res) {
        try {
            ...
            if(!bookmark_collections.length) return res.status(404).json({status: 404, msg: 'Bookmark Collections not found', data: null});    
    ...
    updateBookmarkCollection: async function(req, res) {
        ...
        try {
            ...
            await BookmarkCollection.update({name_collection}, {where: {[Op.and]: [{id: bookmarkCollectionId},{userId}]}});
    ...
    deleteBookmarkCollection: async function (req, res){
        const bookmarkCollectionIds = req.body.ids;

        try {
            await BookmarkCollection.destroy({where: {id: bookmarkCollectionIds}})
            res.status(200).json({status: 200, msg: "Bookmark deleted succesfully", id_bookmark: bookmarkCollectionIds})
        } catch (error) {
            console.error(error);
            res.status(500).json({status: "error", error, msg: error.message, data: null});
        }
